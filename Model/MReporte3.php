...
$sql = "SELECT R.reservaId AS ReservaID, P.nombrePlato AS Plato, DR.cantidad AS Cantidad, DR.subtotal AS Subtotal
FROM reserva R JOIN detallereserva DR ON R.reservaId = DR.reservaId
JOIN plato P ON DR.platoId = P.platoId
ORDER BY R.reservaId ASC
";
// Perform the query
$resultado = mysqli_query($conexion, $sql);
$pdf->Ln(20);
...
$pdf->SetFillColor($rojoClaro, $rojoClaro - 50, $rojoClaro - 50); // Rojo claro
$pdf->Cell(15, 4, 'IDReserva', 1, 0, 'C', true);
$pdf->Cell(20, 4, 'Plato', 1, 0, 'C', true);
$pdf->Cell(20, 4, 'Cantidad', 1, 0, 'C', true);
$pdf->Cell(20, 4, 'Subtotal', 1, 0, 'C', true);
// $pdf->Cell(25, 4, 'Tipo', 1, 0, 'C', true);
// $pdf->Cell(20, 4, 'TotalCobrar', 1, 0, 'C', true);
// $pdf->Cell(20, 4, 'Contraseña', 1, 0, 'C', true);
// $pdf->Cell(15, 4, 'Tipo', 1, 0, 'C', true);
...
while ($mostrar = mysqli_fetch_array($resultado)) {
    $pdf->SetX($centroX);
    $pdf->Cell(15, 4, $mostrar['ReservaID'], 1, 0, 'C');
    $pdf->Cell(20, 4, $mostrar['Plato'], 1, 0, 'C');
    $pdf->Cell(20, 4, $mostrar['Cantidad'], 1, 0, 'C');
    $pdf->Cell(20, 4, '$'.$mostrar['Subtotal'], 1, 0, 'C');
    // $pdf->Cell(25, 4, $mostrar['Tipo'], 1, 0, 'C');
    // $pdf->Cell(20, 4, $mostrar['TotalCobrar'], 1, 0, 'C');
    // $pdf->Cell(20, 4, $mostrar['clave'], 1, 0, 'C');
    // $pdf->Cell(15, 4, $mostrar['tipoUsuario'], 1, 0, 'C');
